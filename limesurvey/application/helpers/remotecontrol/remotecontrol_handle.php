...
class remotecontrol_handle
{
    ...
    */
    ...
    public function get_team_question_id($sSessionKey, $iSurveyID, $questionCode)
    {
        if ($this->_checkSessionKey($sSessionKey))
        {
            $oSurvey = Survey::model()->findByPk($iSurveyID);
            ...
            if (Permission::model()->hasSurveyPermission($iSurveyID, 'survey', 'read')) {
                ...
                ->where(array("and", "sid=:surveyid", "title=:questiontitle"))
                ->from("{{questions}}")
                ->bindParam(":surveyid", $iSurveyID, PDO::PARAM_INT)
                ->bindParam(":questiontitle", $questionCode, PDO::PARAM_STR)
                ->query();
    
                $row = $data->readAll();
    
                return (int) $row[0]["qid"];
            }
            else
                return array('status' => 'No permission');
        }
     ...
     * @param int $iGroupID The id of the group that the question will belong
     */
    public function add_team_answers($sSessionKey, $iSurveyID, $iQuestionID, $aTeamData)
    {
        if ($this->_checkSessionKey($sSessionKey))
        {
            $oSurvey = Survey::model()->findByPk($iSurveyID);
            ...
            if (Permission::model()->hasSurveyPermission($iSurveyID, 'survey', 'update')) {
                foreach ( $aTeamData as $lang => $teamArray )
                {
                    $count = 100;
    
                    foreach ( $teamArray as $teamTitle )
                    {
     ...
     * @param integer $iFromResponseID Optional
     * @param integer $iToResponseID Optional
     * @param array $aTitles Optional Selected field titles
     * @return array|string On success: Requested file as base 64-encoded string. On failure array with error information
     * */
    public function export_custom_responses_by_title($sSessionKey, $iSurveyID, $sDocumentType, $sLanguageCode=null, $sCompletionStatus='all', $sHeadingType='code', $sResponseType='short', $iFromResponseID=null, $iToResponseID=null, $aTitles)
    {
        if (!$this->_checkSessionKey($sSessionKey)) return array('status' => 'Invalid session key');
        Yii::app()->loadHelper('admin/exportresults');
        if (!tableExists('{{survey_' . $iSurveyID . '}}')) return array('status' => 'No Data, survey table does not exist.');
        ...
        // search for title
        $aFields = array();
        foreach ($fieldMap as $fieldKey => $fieldArray) {
            if (!empty($fieldArray['title']) && in_array($fieldArray['title'], $aTitles)) {
                $aFields[] = $fieldKey;
            }
        }
